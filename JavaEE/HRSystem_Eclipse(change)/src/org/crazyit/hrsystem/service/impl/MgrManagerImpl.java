	...
	implements MgrManager
{
	 ...
	 * @param mgr 员工所属的经理
	 */
	public void delEmp(Employee emp , String mgr)throws HrException
	{
		Manager m = mgrDao.findByName(mgr);
		if (m == null)
		{
		...
		throws HrException
	{
		...
		for (Employee e : emps)
		{
			result.add(new EmpBean(e.getName() ,
				e.getPass() , e.getSalary(),e.getSex(),e.getAge(),e.getId()));
		}
		return result;
	}
	
	//加的
	public Employee getEmpsByEmp(String emp)
			throws HrException
	{
		Employee e=empDao.findByName(emp);
